...
class TwichAuthService
{
    ...
    public $fetchUserUrl = 'https://api.twitch.tv/helix/users';
    ...
    public function getUserAttr($accessToken)
    {
        $headers[] = 'Accept: application/json';
        $headers[] = 'Client-ID: ' . $_ENV['TWICH_OAUTH2_CLIENT_ID'];
        $headers[] = 'Authorization: Bearer ' . $accessToken;

        $curl = curl_init($this->fetchUserUrl);
        ...
        return !empty($user->data) ? $user->data[0] : $user->data;
    }

    public function getAccountAttr($token)
    {
        if (isset($token))
        {
            return $this->getUserAttr($token);
        }
        return null;
    }
}