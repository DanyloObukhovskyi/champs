...
class InterkassaStatus
{
     ...
     * @param array $source
     *
     * @see InterkassaStatus::__constructor()
     *
     * @return InterkassaStatus
     */
    public static function factory(InterkassaShop $shop, array $source)
    {
        return new InterkassaStatus($shop, $source);
    }
     ...
     *
     * @throws InterkassaException if some data not received, received shop id
     *                              does not match current shop id or received
     *                              signature is invalid
     */
    public function __construct(InterkassaShop $shop, array $source)
    {
        $this->_shop = $shop;
        ...
        foreach (array(
            ...
            if (!isset($source[$field])) {
                throw new InterkassaException($title . ' not received');
            }
        }
        ...
        if ($received_id !== $shop_id) {
            throw new InterkassaException('Received shop id does not match current shop id');
        }

        if ($this->_checkSignature($source)) {
            $this->_verified = true;
        } else {
            throw new InterkassaException('Signature does not match the data');
        }
     ...
     *
     * Returns {@link InterkassaService::STATE_SUCCESS} or {@link InterkassaService::STATE_FAIL}
     *
     * @return string
     */
