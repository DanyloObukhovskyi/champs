<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Ion_auth
	{
			...
			if ( $this->ion_auth_model->forgotten_password($identity) )   //changed
			{
				...
				if ($user)
				{
					...
					else
					{
						...
						list($customer, $domain) = explode('@', $user->email);
						
						$message = str_replace('{customer_name}', $customer, $message);
						$message = str_replace('{support_email}', $this->config->item('support_email'), $message);
						$message = str_replace('{plugin_name}', $this->config->item('plugin_name'), $message);
						
						$this->email->clear();
						$this->email->from($this->config->item('support_email'), $this->config->item('site_name'));
						$this->email->to($user->email);
						$this->email->subject($this->config->item('site_name') . ' - ' . $this->lang->line('email_forgotten_password_subject'));
		...
		public function get_activate_code($id = NULL) {
			return $this->ion_auth_model->get_activate_code($id);
		}
		 ...
		 * @return bool
		 */
		public function register($identity, $password, $email, $additional_data = array(), $group_ids = array(), $email_activation = true) //need to test email activation
		{
			$this->ion_auth_model->trigger_events('pre_account_creation');
			
			$id = $this->ion_auth_model->register($identity, $password, $email, $additional_data, $group_ids, $email_activation);
			...
			else
			{
				...
				else
				{
					...
					// Fill fields
					list($customer, $domain) = explode('@', $email);
					...
					$this->email->from($this->config->item('support_email'), $this->config->item('site_name'));
					$this->email->to($email);
					$this->email->subject($this->config->item('site_name') . ' - ' . $this->lang->line('email_activation_subject'));
					$this->email->message($message);
