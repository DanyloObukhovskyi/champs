...
<script type="text/x-template" id="streams-template">
    <div id="event-map-pool">
        <label class="label" for="">Стримы</label>
        <div id="event-map-pool-wrapper" style="display: block">
            <div style="display: flex; width: 100%; align-items: center;" v-for="(stream, index) in streams">
                <input type="hidden" :name="`streams[${index}]stream`" :value="stream.stream">
                <input type="hidden" :name="`streams[${index}]icon_id`" :value="stream.icon.id ?? null">
                ...
                <button type="button" @click="deleteStream(index)" class="btn btn-orange remove-video"
        ...
        <div class="add">
            ...
            <div style="width: 40%;">
                <div style="height: 40px; margin-top: 4px; margin-bottom: 7px; width: 100%">
                    <v-select placeholder="Иконка" :options="icons" v-model="icon" label="orig_name">
                        ...
                        <div id="b1" class="main-post-editor-edit">
                            <div class="players-wrapper">
                                <div style="display: flex; width: 100%; align-items: center;"
                                    ...
                                    <input type="hidden"
                                           :name="`teams-attending[${attendingTeamIndex}][players][${index}]`"
                                           :value="player.id">
        ...
        methods: {
            addStream() {
                ...
                if (this.icon === null || this.icon === '') {
                    return Swal.fire({
                        title: 'Упс!',
                        text: 'Необходимо выбрать иконку региона!',
                        icon: 'error',
                        confirmButtonText: 'ОК'
                    })
                }
            ...
            deleteStream(index) {
                this.streams = this.stream.filter((el, i) => {
                    return i !== index;
                })
            }
        },
