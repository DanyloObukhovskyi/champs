<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Ion_auth_model extends CI_Model
	{
		...
		public function disconnect($name, $user_id) {
			if ($name) {
				$sql = 'UPDATE ' . $this->tables['users'] . ' SET ' . $name . '_id = "", ' . $name . '_info = "" WHERE id = ' . intval($user_id);
				$this->db->query($sql);
		...
		public function get_all_users($is_count = false, $limit = [], $where = []) {
			$this->db->select('*');
		...
		public function is_exists_facebook($facebook_id)
		{
		...
		public function is_exists_google($google_id)
		{
			$query = $this->db->select('id')
		...
		public function activate($id, $code = false)
		{
			...
			if ($code !== FALSE)
			{
				$query = $this->db->select([$this->identity_column, 'new_email'])
		...
		public function get_activate_code($id = NULL) {
		 ...
		 * @author Mathew
		 **/
		public function register($identity, $password, $email, $additional_data = array(), $groups = array(), $manual_activation = true)
		{
			...
			$track_number = random_string('alpha', 8);
			$track_number = strtolower($track_number);
		...
		public function get_user_on_tracking_code($track_code) {
			if (!empty($track_code)) {
				$user = $this->db->where('track_number', $track_code)->get($this->tables['users'])->row();
				return $user;
			}
		 ...
		 * @author Mathew
		 **/
		public function login($identity, $password, $remember = FALSE, $social_name = '', $social_id = '')
		{
			$this->trigger_events('pre_login');
			if (!empty($social_name) && !empty($social_id)) {
				$this->trigger_events('extra_where');
				
				$uid = !empty($social_id['logged_in_id']) ? (int)$social_id['logged_in_id'] : 0;
				if ($uid > 0) {
					$query = $this->db->select($this->identity_column . ', email, id, password, active, last_login')
						->where($social_name . '_id', $social_id['id'])
						->where('id', $uid)
						->limit(1)
						->order_by('id', 'desc')
						->get($this->tables['users']);
				...
				if ($query->num_rows() === 1) {
					...
					$this->trigger_events('extra_where');
					$social_info = serialize($social_id);
					...
					if ($query->num_rows() === 1) {
						$sql = 'UPDATE ' . $this->tables['users'] . ' SET ' . $social_name . '_id = "", ' . $social_name . '_info = "" WHERE ' . $social_name . '_id = ' . $this->db->escape($social_id['id']);
						$this->db->query($sql);
						
						$sql = 'UPDATE ' . $this->tables['users'] . ' SET ' . $social_name . '_id = ' . $this->db->escape($social_id['id']) . ', ' . $social_name . '_info = \'' . $social_info . '\' WHERE id = ' . $uid;
						$this->db->query($sql);
						...
						$email = !empty($social_id['email']) ? $social_id['email'] : 'false';
						
						$this->trigger_events('extra_where');
						
						$query = $this->db->select($this->identity_column . ', email, id, password, active, last_login')
							->where('email', $social_id['email'])
							->limit(1)
							->order_by('id', 'desc')
							->get($this->tables['users']);
						...
						if ($query->num_rows() === 1) {
							$sql = 'UPDATE ' . $this->tables['users'] . ' SET ' . $social_name . '_id = ' . $this->db->escape($social_id['id']) . ', ' . $social_name . '_info = \'' . $social_info . '\' WHERE email = ' . $this->db->escape($social_id['email']);
							$this->db->query($sql);
						...
						/*
							...
							$email    = strtolower($social_id['email']);
	
							$additional_data = array(
								'first_name' => $social_id['first_name'],
								'last_name'  => $social_id['last_name'],
								$social_name . '_id' => $social_id['id'],
								$social_name . '_info' => $social_info
							);
	
							if ($this->register($email, $password, $email, $additional_data)) {
								$query = $this->db->select($this->identity_column . ', email, id, password, active, last_login')
									->where('email', $social_id['email'])
									->limit(1)
									->order_by('id', 'desc')
		...
		public function get_user_on_email($email) {
			$email = trim($email);
			return $this->db->where(array('email' => $email))->get($this->tables['users'])->row();
		}
